...
def p_array_plus(p):
    ...
    try:
        ...
        raise T_StopException(p_str_msg="line {} : {}".format(p.lineno(2), str(err)))
    pass
...
def p_array_sacem(p):
    ...
    # print("p_array_sacem")
    if p[3] != "msg":
        raise T_StopException(p_str_msg="line {} : \'{}\': parameter not valid ! ".format(p.lineno(3), p[3]))
    if p[7] != "svl":
        raise T_StopException(p_str_msg="line {} : \'{}\': parameter not valid ! ".format(p.lineno(7), p[7]))
    ...
    if len(l_svl) != 8:
        raise T_StopException(
            p_str_msg="line {} : \'{}\': parameter size error ! ( {} bytes instead of 8 )".format(p.lineno(7), p[7],
                                                                                                  len(l_svl)))
    l_result = sacem(p_bytes_msg=l_msg, p_bytes_svl=l_svl)
...
def p_array_cbc_mac(p):
    ...
    if len(l_keys123) != 3 * 8:
        raise T_StopException(
            p_str_msg="line {} : \'{}\': parameter size error ! ( {} bytes instead of 24 ) ".format(p.lineno(7), p[7],
                                                                                                    len(l_keys123)))
    l_result = cbc_mac_compute(p_bytes_msg=l_msg,
                               p_bytes_key_1=l_keys123[0 * 8:1 * 8],
                               p_bytes_key_2=l_keys123[1 * 8:2 * 8],
                               p_bytes_key_3=l_keys123[2 * 8:3 * 8])
...
def p_array_hex(p):
    ...
    try:
        ...
        raise T_StopException(p_str_msg="line {} : string \'{}\', not hexadecimal".format(p.lineno(3), text))
...
def p_list_expression(p):
    ...
    try:
        if type_id == "UB":
            ...
            raise T_StopException(p_str_msg="line {} : Array format not defined".format(p.lineno(1)))

    except struct.error as err:
        raise T_StopException(p_str_msg="line {} : value \'{}\', ".format(p.lineno(1), p[1]) + str(err))
...
def p_error(p):
    if p:
        raise T_StopException(p_str_msg="line {} : Syntax error at \'{}\'".format(p.lineno, p.value))
        # print("Syntax error at '%s'" % p.value)
    else:
        raise T_StopException(p_str_msg="Syntax error at EOF")
